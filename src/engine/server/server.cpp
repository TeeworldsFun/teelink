...
void CServer::SetClientName(int ClientID, const char *pName)
{
    ...
    //H-Client: Block MineTee Enemy Name
    if (str_comp_nocase(pName, "teeper") == 0 || str_comp_nocase(pName, "zombitee") == 0 || str_comp_nocase(pName, "spidertee") == 0 || str_comp_nocase(pName, "skeletee") == 0 || str_comp_nocase(pName, "teecow") == 0 || str_comp_nocase(pName, "teepig") == 0)
        return;
...
void CServer::InitBot(int ClientID, int BType)
{
    ...
    str_copy(m_aClients[ClientID].m_aClan, "HDP-BOT", MAX_CLAN_LENGTH);
...
{
	if(ClientID < 0 || ClientID >= MAX_CLIENTS || m_aClients[ClientID].m_State < CClient::STATE_READY || !pClan || str_comp_nocase(pClan, "hdp-bot") == 0)
		return;
...
{
	int ClientID = Tick() % (MAX_CLIENTS-MAX_BOTS);
	...
	if(Sys)
	{
		...
		if(Msg == NETMSG_INFO)
		{
			if(m_aClients[ClientID].m_State == CClient::STATE_AUTH)
			{
                ...
                if (str_comp_nocase(GameServer()->GameType(), "MineTee") == 0 || str_comp_nocase(GameServer()->GameType(), "CTF-BREAK") == 0) //H-Client
                {
                    const char *pHClientVersion = Unpacker.GetString(CUnpacker::SANITIZE_CC);
                    if(str_comp(pHClientVersion, GameServer()->HClientNetVersion()) != 0)
                    {
                        char aReason[256];
                        str_format(aReason, sizeof(aReason), "You need H-Client v%s to play in this server. Download it from www.teeworlds.com/forum (Modifications Section)", GameServer()->HClientNetVersion());
                        m_NetServer.Drop(ClientID, aReason);
                        return;
                    }
                }
