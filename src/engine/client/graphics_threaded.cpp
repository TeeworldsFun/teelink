...
int CGraphics_Threaded::LoadTextureRaw(int Width, int Height, int Format, const void *pData, int StoreFormat, int Flags, int forceTextureID)
{
...
int CGraphics_Threaded::LoadTexture(const char *pFilename, int StorageType, int StoreFormat, int Flags, int forceTextureID)
{
...
void CGraphics_Threaded::ScreenshotDirect(const char *pFilename)
{
	...
	if(Image.m_pData)
	{
		...
		png_set_data(&Png, w, h, 8, PNG_TRUECOLOR, (unsigned char *)Image.m_pData); // ignore_convention
		png_close_file(&Png); // ignore_convention
...
void CGraphics_Threaded::SetColor(vec4 color)
{
	dbg_assert(m_Drawing != 0, "called Graphics()->SetColor without begin");
	CColorVertex Array[4] = {
...
// H-Client
int CGraphics_Threaded::GetInvalidTexture() {
    return m_InvalidTexture;
}

void CGraphics_Threaded::TakeScreenshotFree(const char *pFilename, bool tumbtail)
{
