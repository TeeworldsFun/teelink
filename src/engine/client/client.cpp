...
void CClient::ProcessConnlessPacket(CNetChunk *pPacket)
{
	...
	else if(pPacket->m_DataSize >= (int)sizeof(SERVERBROWSE_LIST) &&
		mem_comp(pPacket->m_pData, SERVERBROWSE_LIST, sizeof(SERVERBROWSE_LIST)) == 0)
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			static char IPV4Mapping[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, (char)0xFF, (char)0xFF };
	...
	// server info
	else if(pPacket->m_DataSize >= (int)sizeof(SERVERBROWSE_INFO) && mem_comp(pPacket->m_pData, SERVERBROWSE_INFO, sizeof(SERVERBROWSE_INFO)) == 0)
	{
		// we got ze info
		CUnpacker Up;
		CServerInfo Info = {0};

		// H-Client: DDNet - Don't add info if we already got info64
		...
		if(!Up.Error())
		{
			...
			if (str_find(Info.m_aGameType, "64") || str_find(Info.m_aName, "64"))
			{
...
void CClient::Update()
{
	...
	else if(State() == IClient::STATE_ONLINE && m_RecivedSnapshots >= 3)
	{
		...
		const int64 TimeMult = Freq/50;

		while(1)
		{
			CSnapshotStorage::CHolder *pCur = m_aSnapshots[SNAP_CURRENT];
			int64 TickStart = (pCur->m_Tick)*TimeMult;
		...
		if(m_aSnapshots[SNAP_CURRENT] && m_aSnapshots[SNAP_PREV])
		{
			int64 CurtickStart = (m_aSnapshots[SNAP_CURRENT]->m_Tick)*TimeMult;
			int64 PrevtickStart = (m_aSnapshots[SNAP_PREV]->m_Tick)*TimeMult;
			int PrevPredTick = (int)(PredNow*50/Freq);
			...
			CurtickStart = NewPredTick*TimeMult;
			PrevtickStart = PrevPredTick*TimeMult;
			...
			if(NewPredTick < m_aSnapshots[SNAP_PREV]->m_Tick-SERVER_TICK_SPEED || NewPredTick > m_aSnapshots[SNAP_PREV]->m_Tick+SERVER_TICK_SPEED)
			{
				...
				m_PredictedTime.Init(m_aSnapshots[SNAP_CURRENT]->m_Tick*TimeMult);
			}
			...
			time_get() > m_CurrentServerInfoRequestTime)
		{
			...
			m_CurrentServerInfoRequestTime = time_get()+Freq*2;
		}
	}
