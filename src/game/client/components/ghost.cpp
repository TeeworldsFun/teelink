...
{
	if(!g_Config.m_hcRaceGhost || Client()->State() != IClient::STATE_ONLINE)
		return;
	...
	// Play the ghost
	if(!m_Rendering || !g_Config.m_hcRaceShowGhost)
		return;
...
{
	if(!g_Config.m_hcRaceGhost || Client()->State() != IClient::STATE_ONLINE || m_pClient->m_Snap.m_SpecInfo.m_Active)
		return;
