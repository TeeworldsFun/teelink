...
const CCountryFlags::CCountryFlag *CCountryFlags::GetByCountryCodeName(const char *CountryCodeName) const
{
	for (int i=0; i<m_aCountryFlags.size(); i++) {
        if (str_comp_nocase(m_aCountryFlags[i].m_aCountryCodeString, CountryCodeName) == 0)
