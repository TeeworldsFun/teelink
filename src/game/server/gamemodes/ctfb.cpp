...
CGameControllerCTFB::CGameControllerCTFB(class CGameContext *pGameServer)
: IGameController(pGameServer)
{
	m_apFlags[0] = 0;
	m_apFlags[1] = 0;
	m_pGameType = "CTF-BREAK";
	m_GameFlags = GAMEFLAG_TEAMS|GAMEFLAG_FLAGS;
}

bool CGameControllerCTFB::OnEntity(int Index, vec2 Pos)
{
...
int CGameControllerCTFB::OnCharacterDeath(class CCharacter *pVictim, class CPlayer *pKiller, int WeaponID)
{
...
void CGameControllerCTFB::DoWincheck()
{
...
void CGameControllerCTFB::StartRound()
{
    Server()->ReloadMap();
...
bool CGameControllerCTFB::CanBeMovedOnBalance(int ClientID)
{
...
void CGameControllerCTFB::Snap(int SnappingClient)
{
...
void CGameControllerCTFB::Tick()
{
    ...
    //Water & Lava Envirionment Interact
    static float m_TimeBlocksInter = Server()->Tick();
    if (Server()->Tick() - m_TimeBlocksInter > Server()->TickSpeed()*2.0f)
    {
        ...
        if (pTmap)
        {
            ...
            {
                for(int x = 1; x < pTmap->m_Width-1; x++)
                {
                    int c = y*pTmap->m_Width+x;

                    if ((pTempTiles[c].m_Index == BLOCK_AGUA || pTempTiles[c].m_Index == BLOCK_LAVA) && y+1 < pTmap->m_Height-1)
                    {
                        int tc = (y+1)*pTmap->m_Width+x;
                        if (pTempTiles[tc].m_Index == 0)
                        {
                            ...
                            TileInfo.m_X = x;
                            TileInfo.m_Y = y+1;
                            TileInfo.m_ITile = pTiles[c].m_Index;
                            TileInfo.m_Col = 0;
                            TileInfo.m_Act = TILE_CREATE;
                            TileInfo.m_State = 0;

                            Server()->SendPackMsg(&TileInfo, MSGFLAG_VITAL, -1);
                            GameServer()->Collision()->CreateTile(vec2(TileInfo.m_X<<5, TileInfo.m_Y<<5), pTiles[c].m_Index, 0);
                        }
                        else if ((pTempTiles[c].m_Index == BLOCK_LAVA && pTempTiles[tc].m_Index == BLOCK_AGUA) || (pTempTiles[c].m_Index == BLOCK_AGUA && pTempTiles[tc].m_Index == BLOCK_LAVA))
                        {
                            CNetMsg_Sv_TileChangeExt TileInfo;
                            TileInfo.m_Size = -1;
                            TileInfo.m_Index = -1;
